...
private extension SwiftGrammar {
    static let keywords = ([
        ...
        "override", "where", "default", "break", // "_"
    ...
    static let builtInTypes: Set<String> = [
        "Array", "ArraySlice", "Bool", "Character", "ClosedRange", "ClosedRangeIndex",
        "CodingKey", "CodingUserInfoKey", "Collection", "Decodable", "Decoder",
        "Dictionary", "Double", "EmptyCollection", "Encodable", "Encoder", "Error",
        "Float", "Hashable", "Int", "Int16", "Int32", "Int64", "Int8", "Optional",
        "Range", "Sequence", "Set", "Slice", "Strideable", "String", "StringProtocol",
        "UInt", "UInt16", "UInt32", "UInt64", "UInt8", "URL"
    ]
    
    static let builtInFunctions: Set<String> = [
        "print", "assert", "assertionFailure", "fatalError",
        "abs", "cos", "sin", "tan", "min", "max", "abort",
        "exit"
    ]
    ...
    struct CallRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            guard !segment.tokens.onSameLine.last.isAny(of: ".") else {
                return false
            }
    ...
    struct TypeRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            if let previousToken = segment.tokens.previous {
                ...
                guard !segment.tokens.onSameLine.contains(anyOf: keywords) else {
                    return false
                }
            }
    ...
    struct OtherDeclarationRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            guard segment.tokens.previous.isAny(of: "func", "var", "let", "some", "case") else {//"for"
                return false
            }
    ...
    struct ParameterRule: SyntaxRule {
        ...
        func matches(_ segment: Segment) -> Bool {
            guard let last = segment.tokens.all.lastIndex(where: { $0.isAny(of: "func", "init") }) else {
                return false
            }
    ...
    struct InitialiserRule: SyntaxRule {
        var tokenType: TokenType { return .dotAccess }
        ...
        func matches(_ segment: Segment) -> Bool {
            ...
            return segment.isWithinParanthesis
        }
    }
    
    struct InheritanceRule: SyntaxRule {
        var tokenType: TokenType { return .otherDeclaration }
